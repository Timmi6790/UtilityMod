...
public class BaseModule implements Module {
    ...
    public void registerEvents() {
        ...
        for (final ListenerComponent listenerComponent : this.listenerComponents) {
            try {
                log.debug(
                        "Registering listener component {} in module {}.",
                        listenerComponent.getClass().getSimpleName(),
                        this.getClass().getSimpleName());
                listenerComponent.registerEvents();
            } catch (final Exception e) {
                log.error(
                        "Failed to register listener component: "
                                + listenerComponent.getClass().getSimpleName(),
                        e);
            }
        }
    ...
    public void disable() {
        ...
        for (final ListenerComponent listenerComponent : this.listenerComponents) {
            try {
                log.debug(
                        "Unregistering listener component {} in module {}.",
                        listenerComponent.getClass().getSimpleName(),
                        this.getClass().getSimpleName());
                listenerComponent.unregisterEvents();
            } catch (final Exception e) {
                log.error(
                        "Failed to unregister listener component: "
                                + listenerComponent.getClass().getSimpleName(),
                        e);
            }
        }
        ...
        for (final BaseCommand command : this.commands) {
            try {
                log.debug(
                        "Unregistering command {} in module {}.",
                        command.getCommandName(),
                        this.getClass().getSimpleName());
                command.unregister();
            } catch (final Exception e) {
